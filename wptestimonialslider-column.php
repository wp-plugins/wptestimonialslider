...
function wpt_screen_columns( $columns ) {
	unset( $columns['date'] );
...
// GET FEATURED IMAGE
function wpt_featured_image($post_ID) {
...
add_action('manage_posts_custom_column', 'wpt_columns_content', 10, 2);
// SHOW THE FEATURED IMAGE
function wpt_columns_content($column_name, $post_ID) {
    if ($column_name == 'featured_image') {
        $post_featured_image = wpt_featured_image($post_ID);
...
add_action( 'manage_posts_custom_column', 'wpt_populate_columns' );

function wpt_populate_columns( $column ) {
...
// Columns as Sortable
add_filter( 'manage_edit-wptestimonialslider_sortable_columns', 'wpt_sort' );

function wpt_sort( $columns ) {
